...
def get_distance(feature1, feature2, mask1, mask2, RESOLUTION=64):
    ...
    src_feature_upsampled[src_feature_upsampled == 0] = 0.0
    tgt_feature_upsampled[tgt_feature_upsampled == 0] = 0.0
...
def calculate_keypoint_transformation(args, img1_desc, img2_desc, img1_patch_idx, num_patches, sph_feature=False, img1_desc_sph=None, img2_desc_sph=None, w_sph=0.2):
    """
    ...
    sim_1_to_2 = torch.matmul(img1_desc, img2_desc.permute(0, 2, 1))[0]  # [3600, 3600] # -> [3600, n*3600]

    if sph_feature:
        sim_1_to_2_sph = torch.matmul(img1_desc_sph[0], img2_desc_sph[0].permute(0, 2, 1))[0]  # [3600, 3600]
        sim_1_to_2 = (1-w_sph)*sim_1_to_2 + w_sph*sim_1_to_2_sph

    sim_1_to_2_idxed = sim_1_to_2[img1_patch_idx]
    anno_stride = args.ANNO_SIZE / num_patches

    # Here the SOFT_EVAL is happening.
    if args.SOFT_EVAL:
...
def kpt_to_patch_idx(img1_kps, num_patches, anno_size=840):
    img1_y, img1_x = img1_kps[:, 1].numpy(), img1_kps[:, 0].numpy()
    img1_y_patch = (num_patches / anno_size * img1_y).astype(np.int32)
    img1_x_patch = (num_patches / anno_size * img1_x).astype(np.int32)
    img1_patch_idx = num_patches * img1_y_patch + img1_x_patch
    return img1_patch_idx


def normalize_feats(feats, sd_and_dino=False, epsilon=1e-10):
    if sd_and_dino: # includes SD+DINOv2
...
def load_features_and_masks(aggre_net, img_path, flip, ensemble, num_patches, device,img_dir,feature_dir,only_dino=False):
    ...
    if img_dir != '' and feature_dir != '':
        feature_base = img_path.replace(img_dir, feature_dir).replace(f".{f_img_ending}", '')
    feature_base = img_path.replace('JPEGImages', 'features').replace(f".{f_img_ending}", '')
...
def forward_feats(aggre_net,img_path_1,img_dir,feature_dir, sd_and_dino=False, only_dino=False, num_patches=60, device='cpu'):
    with torch.no_grad():
        img1_desc, mask1 = load_features_and_masks(aggre_net, img_path_1, False, 1, num_patches, device,img_dir,feature_dir,only_dino=only_dino);
        img1_desc = normalize_feats(img1_desc,sd_and_dino=sd_and_dino)
