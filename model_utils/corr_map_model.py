...
class Correlation2Displacement(nn.Module):
    ...
    def soft_argmax(self, corr, beta=0.02, h_t=None,w_t=None):
        r'''SFNet: Learning Object-aware Semantic Flow (Lee et al.)'''
        # input shape : (B, H_t * W_t, H_s , W_s) e.g., (B, 32*32, 32, 32)
        b,_,h,w = corr.size() 
        h_t = h_t if h_t is not None else h
        w_t = w_t if w_t is not None else w
        
        corr = self.softmax_with_temperature(corr, beta=beta, d=1) # (B, H_t * W_t, H_s , W_s)
        corr = corr.view(-1,h_t,w_t,h,w) # (target hxw) x (source hxw)

        grid_x = corr.sum(dim=1, keepdim=False) # marginalize to x-coord.
        x_normal = self.x_normal.expand(b,w_t)
        x_normal = x_normal.view(b,w_t,1,1)
        grid_x = (grid_x*x_normal).sum(dim=1, keepdim=True) # b x 1 x h x w
        
        grid_y = corr.sum(dim=2, keepdim=False) # marginalize to y-coord.
        y_normal = self.y_normal.expand(b,h_t)
    ...
    def forward(self, x, ):
        ...
        # take soft argmax
        grid_x, grid_y = self.soft_argmax(x.permute(0, 3, 1, 2),h_t=h_t, w_t=w_t)
